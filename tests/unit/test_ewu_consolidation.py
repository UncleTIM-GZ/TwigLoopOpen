...
class TestEwuCapEnforcement:
    """Verify EWU > 8 is blocked at config level; schema no longer accepts ewu."""

    def test_quota_config_max_ewu_per_task(self) -> None:
        assert MAX_EWU_PER_TASK == 8

    def test_create_schema_rejects_ewu_field(self) -> None:
        """CreateTaskCardRequest no longer accepts an ewu field (extra=forbid)."""
...
class TestSchemaDefaultConsistency:
    ...
    def test_create_schema_has_no_ewu_field(self) -> None:
        """EWU is no longer part of the create schema; it defaults in the service layer."""
        assert "ewu" not in CreateTaskCardRequest.model_fields
