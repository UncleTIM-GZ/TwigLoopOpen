...
class TaskCardService:
    ...
    async def create(
        ...
        # EWU defaults to 1.00 at creation; only calculate-ewu can set it.
        has_reward = project.has_reward if project else False
    ...
    async def update(
        ...
        allowed = {"title", "goal", "output_spec", "completion_criteria"}
    ...
    async def transition_status(
        ...
        # Completion gate: "submitted" requires at least 1 evidence
        if target_status == "submitted":
            evidence_count = await self._evidence_repo.count_by_task(task_id)
            if evidence_count < 1:
                raise ConflictError(
                    "Cannot submit task without at least one piece of delivery evidence"
                )

        # Completion gate: "completed" requires verified or legacy mode
        if (
            ...
            raise ConflictError(
                "Cannot complete task: verification_status must be 'verified' "
                "or completion_mode must be 'legacy'"
            )
