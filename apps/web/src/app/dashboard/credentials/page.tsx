    ...
    <div className="bg-gray-950 rounded-xl p-6 md:p-8 space-y-8 text-gray-100">
      ...
      <div className="border-b border-gray-800 pb-6">
        ...
        <p className="text-sm text-gray-500 font-mono">
          verifiable credentials issued to {actor.display_name}
        </p>
      </div>
      ...
      {credentials.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {credentials.map((cred) => (
            <CredentialRow key={cred.credential_id} credential={cred} />
          ))}
        </div>
      )}
...
function CredentialRow({ credential }: { credential: CredentialResponse }) {
  ...
  return (
    <div className="rounded-lg border border-gray-800 bg-gray-900 p-4 flex items-center justify-between gap-4">
      ...
      <a
        ...
        className="shrink-0 rounded-md border border-gray-700 px-3 py-1.5 text-xs font-mono text-gray-300 hover:border-green-400 hover:text-green-400 transition-colors"
      >
        verify
      </a>
    </div>
