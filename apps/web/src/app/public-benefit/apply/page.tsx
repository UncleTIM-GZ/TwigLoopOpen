...
export default function PublicBenefitApplyPage() {
  ...
  const isLoggedIn = !!actor;
  const isFounder = !!actor?.is_founder;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await apiClient<ApiResponse<{ application_id: string }>>(
        "/api/v1/public-benefit/apply",
        {
          method: "POST",
          body: JSON.stringify({
            ...
            problem_to_solve: form.problemToSolve,
            target_audience: form.targetAudience || undefined,
            desired_help: form.desiredHelp,
            existing_resources: form.existingResources || undefined,
            repo_link: form.repoLink || undefined,
            additional_notes: form.additionalNotes || undefined,
            file_link: form.fileLink || undefined,
          }),
        },
      );
      if (res.data) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-gray-400 text-sm text-center max-w-md">
          Please log in or register to submit a public benefit request.
        </p>
        <div className="flex gap-4">
          <Link
            href="/login"
            className="rounded-md border border-gray-700 px-4 py-2 text-sm text-gray-300 hover:border-green-400 hover:text-green-400 transition-colors"
          >
            Log In
          </Link>
          <Link
            href="/register"
            className="rounded-md bg-green-400 px-4 py-2 text-sm font-medium text-gray-950 hover:bg-green-300 transition-colors"
          >
            Register
          </Link>
        </div>
  ...
  // Logged in but not founder -- show MCP suggestion
  if (!isFounder) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        <h1 className="font-mono text-2xl font-bold text-green-400 mb-2">
        ...
        <p className="text-gray-400 text-sm text-center max-w-md">
          Public benefit requests are currently available to founders. You can
          use MCP to request founder access or initiate a project first.
        </p>
        <Link
          href="/create"
          className="rounded-md border border-gray-700 px-4 py-2 text-sm text-gray-300 hover:border-green-400 hover:text-green-400 transition-colors"
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 font-mono mb-4">
        Submitting as: {actor.display_name}
      </p>
      ...
      {error && (
        <div className="mb-6 rounded-md border border-red-400/30 bg-red-400/5 px-4 py-3">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-8">
        ...
        <button
          type="submit"
          disabled={submitting}
          className="self-start rounded-md bg-green-400 px-6 py-2.5 text-sm font-medium text-gray-950 hover:bg-green-300 transition-colors disabled:opacity-50"
        >
